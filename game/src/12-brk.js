...
/* A four-seater, wide enough that nobody sits in anybody's lap.  It is
   drawn in two passes — the back, then the people, then the seat over
   their thighs — so everyone reads as sitting rather than standing in
   front of the furniture. */
const COUCH_W = 236;                       // half-width of the seat
function couchBack(x, y, s){
  g.save(); g.translate(x,y); g.scale(s,s);
  g.fillStyle='rgba(0,0,0,.25)';
  g.beginPath(); g.ellipse(0,4,COUCH_W+18,14,0,0,7); g.fill();
  g.fillStyle='#4a5f72'; rr(-COUCH_W,-104,COUCH_W*2,84,14); g.fill();
  g.fillStyle='#415464'; rr(-COUCH_W,-104,COUCH_W*2,11,6); g.fill();
  g.strokeStyle='rgba(0,0,0,.20)'; g.lineWidth=2;
  for (let i=1;i<4;i++){
    const sx = -COUCH_W + i*(COUCH_W*2/4);
    g.beginPath(); g.moveTo(sx,-104); g.lineTo(sx,-22); g.stroke();
  }
  g.restore();
}
function couchFront(x, y, s){
  g.save(); g.translate(x,y); g.scale(s,s);
  // the seat slab, drawn over everyone's thighs
  g.fillStyle='#5a7186'; rr(-COUCH_W,-46,COUCH_W*2,50,10); g.fill();
  g.fillStyle='#65798f'; rr(-COUCH_W,-46,COUCH_W*2,9,6); g.fill();
  g.fillStyle='rgba(0,0,0,.14)'; rr(-COUCH_W,-2,COUCH_W*2,6,3); g.fill();
  g.strokeStyle='rgba(0,0,0,.20)'; g.lineWidth=2;
  for (let i=1;i<4;i++){
    const sx = -COUCH_W + i*(COUCH_W*2/4);
    g.beginPath(); g.moveTo(sx,-46); g.lineTo(sx,2); g.stroke();
  }
  // arms
  g.fillStyle='#3d5164'; rr(-COUCH_W-18,-70,28,74,9); g.fill();
  g.fillStyle='#46596b'; rr(-COUCH_W-18,-70,28,10,6); g.fill();
  g.fillStyle='#3d5164'; rr( COUCH_W-10,-70,28,74,9); g.fill();
  g.fillStyle='#46596b'; rr( COUCH_W-10,-70,28,10,6); g.fill();
  // feet
  g.fillStyle='#2f4050';
  rr(-COUCH_W+12,4,16,14,3); g.fill(); rr(COUCH_W-28,4,16,14,3); g.fill();
  g.restore();
}
...
/* where the couch is, and where each person sits on it */
const COUCH_X = 880;
const SEAT = [COUCH_X - 174, COUCH_X - 58, COUCH_X + 58, COUCH_X + 174];
const SEAT_Y = 640;   // feet on the floor in front, thighs behind the seat
...
const S_brk = {
  ...
  draw(){
    ...
    drawPerson(stu,  SEAT[0], SEAT_Y, 1.7, { dir:'right', face:'neutral', seed:11,
                                             pose:'slump', shadow:false });
    drawPerson(stu2, SEAT[3], SEAT_Y, 1.7, { dir:'left',  face:'happy',   seed:17,
                                             pose:'slump', shadow:false });

    /* ---------------- Dina ---------------- */
    const dinaX = SEAT[1];
    const dinaTalk = this.convo && this.convo.speaking(NPCS.colleague.name);
    drawPerson(NPCS.colleague, dinaX, SEAT_Y, 1.72, { shadow:false,
      dir: this.px < dinaX ? 'left' : 'right',
      face: this.mode==='wake' ? 'shock' : 'happy',
      talk: dinaTalk, seed:9,
      pose: dinaTalk ? 'talk' : 'idle'
    });
    txt(NPCS.colleague.name, dinaX, SEAT_Y + 22, 13, 'rgba(234,244,250,.5)');
    ...
    /* once seated, the engineer sits on the couch like everyone else */
    const seated = (this.mode === 'sleep' || this.mode === 'wake');
    let hx = this.px, hy = this.py, hs = 1.95;
    if (seated){ hx = SEAT[2]; hy = SEAT_Y; hs = 1.72; }
    if (this.mode === 'sleep'){
      hopt = Object.assign(hopt, { dir:'right', walk:0, face:'sleep',
        headTilt:0.42, pose:'slump', shadow:false });
      this.zT += dt;
      if (this.zT > 34){ this.zT = 0; emitZ(hx+26, hy-190); }
    } else if (this.mode === 'wake'){
      hopt = Object.assign(hopt, { dir:'right', walk:0,
        face: this.gaspT>0 ? 'panic' : 'worry',
        sweat: this.gaspT>0 ? 3 : 1, pose:'panic', shadow:false });
      if (this.gaspT>0) this.gaspT -= dt;
    } else if (this.mode === 'chat'){
      hopt.face = 'happy';
    }
    drawPerson(hero, hx, hy, hs, hopt);

    /* the seat cushion goes over everybody's thighs */
    couchFront(COUCH_X, 604, 1.0);
    ...
    if (this.mode === 'sleep'){
      txt('...', SEAT[2]+42, SEAT_Y-216, 26, 'rgba(200,220,245,.6)');
    }
  ...
  answer(k){
    ...
    this.convo = Convo([
      ...
      { by:NPCS.colleague.name, text:hers, at:()=>[SEAT[1], 372], w:330 },
